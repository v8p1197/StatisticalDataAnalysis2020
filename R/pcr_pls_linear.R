...
pcr.fit=pcr(fit.linear,data = merComplete ,scale=TRUE, validation ="CV")
...
set.seed (1)
x = model.matrix(fit.linear,data = merComplete)[,-1]
...
pcr.fit=pcr(fit.linear,data = merComplete ,subset=train,scale=TRUE,
            validation ="CV")
dev.new()
# Plot MSE and RMSE 
validationplot(pcr.fit,val.type="MSEP",legendpos = "topright") 
...
# Setting scale=TRUE has the effect of standardizing each predictor (scale projection).
# Setting validation = 'CV' has the effect to use cross validation to rate M parameter
pls.fit=plsr(fit.linear,data = merComplete, scale=TRUE, 
             validation ="CV")
summary(pls.fit)
dev.new()
...
set.seed (1)
pls.fit=plsr(fit.linear,data = merComplete, subset=train, 
...
pls.fit=plsr(fit.linear,data = merComplete ,scale=TRUE,ncomp=8)
summary(pls.fit)
